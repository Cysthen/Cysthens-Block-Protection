...
function BP_on()
    ...
    if ( DEFAULT_CHAT_FRAME ) then
      DEFAULT_CHAT_FRAME:AddMessage("Divine Protection will be removed.");
    end
end
...
function BP_off()
    ...
    if ( DEFAULT_CHAT_FRAME ) then
      DEFAULT_CHAT_FRAME:AddMessage("Divine Protection will not be removed.");
    end
end
...
function BP_Kill() 
	...
	if ((string.find(texture,"Spell_Holy_Restoration"))) then
	    CancelPlayerBuff(i);
		DEFAULT_CHAT_FRAME:AddMessage("Removed Divine Protection.");		
	    return true;
	end
    i = i + 1;
    end
